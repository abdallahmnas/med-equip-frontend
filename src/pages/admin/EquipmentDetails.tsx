...
export function EquipmentDetails() {
  ...
  const handleAddSpecification = async (spec: { name: string, value?: string, rangeFrom?: string, rangeTo?: string }) => {
            ...
            <div className="md:col-span-2 space-y-6">
              <Card className="p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{equipmentDetails.category?.name || 'N/A'}</p>
                  </div>
                </div>
              </Card>
              ...
              <Card className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-semibold">Specifications & Keywords</h2>
                  <Button
                    variant="outline"
                    size="sm"
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  {equipmentDetails.properties.map((spec, index) => (
                    <div key={index} className="relative group p-4 rounded-lg border hover:border-destructive/50 transition-colors">
                      <Button
                        variant="ghost"
                        size="icon"
                        className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity"
                        onClick={() => setDeletingSpec({ id: spec.id, name: spec.name })}
                      >
                  ...
                  {equipmentDetails.images.map((image, index) => (
                    <div key={index} className="relative group">
                      <Button
                        ...
                        onClick={() => setDeletingImage({ id: image.id, index: index + 1 })}
                      >
                        <Trash2 className="h-4 w-4 text-destructive" />
                      </Button>
